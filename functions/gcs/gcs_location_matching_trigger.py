...
def process_created(data, context):
    try:
        ...
        validation_fields = {'sic code': (False, 'sic_code'), 'chain name/chain id': (True, 'chain'),
        ...
        temp_table = f'store_list_full_address'
        ...
        add_clean_fields = f""" CREATE TEMP FUNCTION cleanStr(str string, type string) RETURNS string
                              LANGUAGE js AS \"\"\"
                                return cleanStr(str, type)
                            \"\"\"
                            OPTIONS (library=['gs://javascript_lib/addr_functions.js']);
                            create or replace table {dataset}.{temp_table} as 
                            select *, cleanstr(chain_name, 'chain') clean_chain, 
                            cleanstr(street_address, 'addr') clean_addr
...
# process_created({'name': 'dviorel/match_multiple_sic_codes.txt'}, None)      # No chain names
